...
fn generated_sources_are_up_to_date() {
    ...
    let checked_in = workspace_dir.join("bnd-openssl/src/openssl");

    // Generate into a temp directory structured like bnd-openssl
    let tmp = tempfile::tempdir().unwrap();
    let tmp_src = tmp.path().join("src/openssl");
    ...
    bnd_openssl_gen::generate(tmp.path());

    let generated_dir = tmp.path().join("src/openssl");
    ...
    assert!(
        diffs.is_empty(),
        "The following checked-in files are out of date. Run `cargo run -p bnd-openssl-gen` \
         to regenerate:\n  {}",
        diffs.join("\n  ")
    );
}
